                    ...
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($res)): ?>
                            <tr>
                                ...
                                <td>
                                    <?php if ($row['image']): ?>
                                        <img src="../<?php echo htmlspecialchars($row['image']); ?>" width="80" alt="Product Image" />
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <a href="edit_product.php?id=<?php echo $row['id']; ?>" class="action-link edit-link">Edit</a> |
                                    <a href="delete_product.php?id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this product?')" class="action-link delete-link">Delete</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
